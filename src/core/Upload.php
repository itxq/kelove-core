 ...
 * @package util
 */
class Upload extends SingleModel
{
    /**
     * 图片转Base64
     ...
     * @return bool|string
     */
    public function imgTurnBase64($url, $extension = '') {
        try {
     ...
     * @return bool|string
     */
    public function getRemoteFile($url, $savePath = '', $saveName = '', $config = []) {
     ...
     * @return bool
     */
    public function thumb($tempDir, $saveDir, $saveName, $width = 100, $height = 100) {
        if (!is_dir($saveDir)) {
     ...
     * @return bool
     */
    private function saveImg($content, $savePath, $saveName, $extension) {
        /* 创建目录 */
        if (!is_dir($savePath)) {
